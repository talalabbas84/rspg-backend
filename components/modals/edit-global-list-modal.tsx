...
// No direct hook import needed if onUpdate is passed from parent that uses the hook

interface EditGlobalListModalProps {
  list: GlobalList | null
  open: boolean
  onOpenChange: (open: boolean) => void
  onUpdate: (list: GlobalList) => Promise<void> // Parent will provide this, connected to useGlobalLists hook
}
...
export function EditGlobalListModal({ list, open, onOpenChange, onUpdate }: EditGlobalListModalProps) {
  ...
  useEffect(() => {
    if (list) {
      ...
      setValues(list.values && list.values.length > 0 ? list.values : [""])
    } else {
      // Reset form when list is null (modal is closing or list cleared)
      setName("")
      setDescription("")
      setValues([""])
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedList: GlobalList = {
        ...
        updated_at: new Date().toISOString(), // Frontend can set this, backend might override
      }
      await onUpdate(updatedList) // Call parent's update function
      // onOpenChange(false) // Parent should handle closing on successful update
    } catch (error) {
      // Error toast should be handled by the parent's onUpdate function (which uses the hook)
      console.error("Failed to update list from modal:", error)
    } finally {
  ...
  if (!list) return null // Don't render if no list is provided
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle>Edit Global List: {list.name}</DialogTitle>
